...
## Важные фиксы
### v1.20.13 — Ускорение автозаполнения
- Интервал ретраев в `waitAndFill` / `waitForPac` уменьшен **300 мс → 100 мс**
- Поля и дропдауны подхватываются в 3 раза быстрее; протестировано в бою ~30 мин без сбоев
